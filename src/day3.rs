...
fn get_priority_hashmap() -> HashMap<char, usize> {
    ...
    for letter in &alphabet {
        priorities.insert(*letter, alphabet.iter().position(|&x| x == *letter ).unwrap() + 1);
        println!("Added enty to hashmap: {} -> {}", letter, priorities[&letter]);
    }
    ...
    for ruck in input.lines() {
        ...
        for item in &comp1 {
            if comp2.contains(item) {
                ...
                println!("Found the repeat item: {}, adding {} to the priority sum.", *item, priorities_map[item]);
